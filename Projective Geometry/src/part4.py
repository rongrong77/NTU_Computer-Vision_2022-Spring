...
def panorama(imgs):
    ...
    for idx in range(len(imgs) - 1):
        ...
        # TODO: 5. apply alpha blending
        
        def alpha_blending(im1, im2, alpha):
           
            blended = alpha * im1 + (1 - alpha) * im2
            blended = blended.astype(np.uint8)
            return blended
        im1_resized = cv2.resize(im1, (dst.shape[1], dst.shape[0]))
        im2_resized = cv2.resize(im2, (dst.shape[1], dst.shape[0]))
            
        # Convert the warped image to float32 for alpha blending
        dst_float = dst.astype(np.float32)
            
        # Calculate the alpha value based on the pixel intensities
        alpha = np.where(dst_float == 0, 0, 1)
            
        # Apply alpha blending between the resized images and the warped image
        blended = alpha * im1_resized + (1 - alpha) * im2_resized
        blended = blended.astype(np.uint8)
            
        # Update the warped image with the blended result
        dst = blended
