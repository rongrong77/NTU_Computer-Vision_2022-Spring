...
class Joint_bilateral_filter(object):
    ...
    def joint_bilateral_filter(self, img, guidance):
        ...
        # Step 1: Calculate Gs -- constant matrix
        ...
        # Step 3: Calculate Gr with guidance(gray), G:=Gr*Gs, JB-filter I->I'
        ...
        for i in range(self.pad_w, padded_guidance.shape[0]-self.pad_w):
            for j in range(self.pad_w, padded_guidance.shape[1]-self.pad_w):
                # Step 3-1: Calculate Gr with guidance(gray)
                ...
                for k in range(img.shape[2]):
                    output[i-self.pad_w, j-self.pad_w, k] = np.multiply(G, Iq[:,:,k]).sum() / W
